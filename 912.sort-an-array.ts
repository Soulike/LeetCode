...
class Heap<T>
{
    ...
    private addOne(element: T): void
    {
        ...
        while (elementIndex > 0 && this.compareFunction(this.treeNodes[elementIndex], this.treeNodes[parentIndex]) < 0)
        {
    ...
    public deleteRoot(): void | never
    {
        ...
        else
        {
            ...
            let minChildIndex = this.treeNodes[leftChildIndex] > this.treeNodes[rightChildIndex]
                ? rightChildIndex
                : leftChildIndex;

            while (this.compareFunction(this.treeNodes[minChildIndex], this.treeNodes[elementIndex]) < 0)
            {
                ...
                {
                    minChildIndex = this.treeNodes[leftChildIndex] > this.treeNodes[rightChildIndex]
                        ? rightChildIndex
                        : leftChildIndex;
                }
 ...
 * @param compareFunction - 比较函数。如果 a - b <= 0 则无需交换，反之则需要交换
 */
function heapSort<T>(array: T[], compareFunction: (a: T, b: T) => number): void
{
