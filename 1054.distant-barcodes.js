...
const rearrangeBarcodes = function (barcodes)
{
    ...
    for (const barcode of barcodes) // 统计频率
    {
        barcodeToCount.set(barcode,
            (barcodeToCount.get(barcode) ?? 0) + 1);
    }

    // 频率按照从高到低排列
    const barcodeToCountArray = Array.from(barcodeToCount).sort((a, b) => b[1] - a[1]);
