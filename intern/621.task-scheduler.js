...
const leastInterval = function (tasks, n) {
  ...
  const taskToCountEntries = Array.from(taskToExecutionCount.entries()).sort(
    (a, b) => b[1] - a[1],
  );
  /** 最多的那种任务的执行次数 */
  ...
  /** 除了个数最多的任务之外，剩余任务的数量 */
  const leftTasksAmount =
    tasks.length - tasksWithBiggestExecutedCountAmount * biggestExecutedCount;
  const slotsAmountPerGroup = n + 1;
  const idleSlotsAmount =
    (slotsAmountPerGroup - tasksWithBiggestExecutedCountAmount) *
    (biggestExecutedCount - 1);

  if (idleSlotsAmount >= leftTasksAmount) {
    return (
      slotsAmountPerGroup * (biggestExecutedCount - 1) +
      tasksWithBiggestExecutedCountAmount
    );
  } else {
    return tasks.length;
