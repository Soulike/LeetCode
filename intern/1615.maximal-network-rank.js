...
const maximalNetworkRank = function (n, roads) {
  ...
  for (const [a, b] of roads) {
    cityToConnectedRoadCount.set(a, (cityToConnectedRoadCount.get(a) ?? 0) + 1);
    cityToConnectedRoadCount.set(b, (cityToConnectedRoadCount.get(b) ?? 0) + 1);
    connected[a][b] = true;
    connected[b][a] = true;
  }
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      maxRoadCountBetweenCities = Math.max(
        ...
        (cityToConnectedRoadCount.get(i) ?? 0) +
          (cityToConnectedRoadCount.get(j) ?? 0) +
          (connected[i][j] ? -1 : 0),
      );
    }
  }
