...
var lastStoneWeight = function (stones) {
    const heap = new Heap((a, b) => b - a, stones);

    while (heap.getSize() > 1) {
        const stone1 = heap.getRoot();
        heap.deleteRoot();
        const stone2 = heap.getRoot();
        heap.deleteRoot();

        if (stone1 !== stone2) {
            heap.addOne(Math.abs(stone2 - stone1));
        }
    }

    return heap.getSize() === 1 ? heap.getRoot() : 0;
};
// @lc code=end
