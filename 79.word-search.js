...
const exist = function (board, word) 
{
    ...
    for (let x = 0; x < ROW_AMOUNT; x++)
    {
        ...
        {
            if (board[x][y] === FIRST_CHAR && helper(board, word, 0, visitedMatrix, x, y, ROW_AMOUNT, COL_AMOUNT))
            {
                return true;
            }
        }
 ...
 * @return {boolean}
 */
function helper(board, word, wordIndex, visitedMatrix, currentX, currentY, ROW_AMOUNT, COL_AMOUNT)
{
    ...
    if (wordIndex === word.length - 1 && CURRENT_CHAR === board[currentX][currentY])
    {
        return true;
    }
    ...
    if (currentX > 0)   // up
    {
        if (helper(board, word, nextWordIndex, visitedMatrix, currentX - 1, currentY, ROW_AMOUNT, COL_AMOUNT))
        {
            return true;
        }
    }
    if (currentX < ROW_AMOUNT - 1) // down
    {
        if (helper(board, word, nextWordIndex, visitedMatrix, currentX + 1, currentY, ROW_AMOUNT, COL_AMOUNT))
        {
            return true;
    ...
    if (currentY > 0)   // left
    {
        if (helper(board, word, nextWordIndex, visitedMatrix, currentX, currentY - 1, ROW_AMOUNT, COL_AMOUNT))
        {
            return true;
        }
    }
    if (currentY < COL_AMOUNT - 1)    // right
    {
        if (helper(board, word, nextWordIndex, visitedMatrix, currentX, currentY + 1, ROW_AMOUNT, COL_AMOUNT))
        {
            return true;
