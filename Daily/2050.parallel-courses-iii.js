...
var minimumTime = function (n, relations, time) {
    ...
    /** @type {number[][]} */
    const nextCoursesArray = new Array(n + 1);

    for (const [prev, next] of relations) {
        inDegrees[next]++;

        if (nextCoursesArray[prev] === undefined) {
            nextCoursesArray[prev] = [];
        }
        nextCoursesArray[prev].push(next);
    }

    /** @type {number[]} */
    const inDegreeZeroQueue = [];
    for (let i = 0; i < inDegrees.length; i++) {
        if (inDegrees[i] === 0) inDegreeZeroQueue.push(i);
    }

    const maxFinishTimes = [-Infinity, ...time];

    while (inDegreeZeroQueue.length > 0) {
        /** @type {number} */
        const course = inDegreeZeroQueue.shift();

        const nextCourses = nextCoursesArray[course] ?? [];
        for (const nextCourse of nextCourses) {
            inDegrees[nextCourse]--;
            if (inDegrees[nextCourse] === 0) inDegreeZeroQueue.push(nextCourse);

            maxFinishTimes[nextCourse] = Math.max(
                maxFinishTimes[nextCourse],
                maxFinishTimes[course] + time[nextCourse - 1],
            );
        }
    }

    return Math.max(...maxFinishTimes);
};
// @lc code=end
