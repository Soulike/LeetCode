    ...
    while (numToFreq.size > 0)
    {
        ...
        for (const num of leftNums)
        {
            if ((nextNeededNum.get(num) ?? 0) > 0)
            {
                ...
                nextNeededNum.set(num + 1,
                    (nextNeededNum.get(num + 1) ?? 0) + 1);
                break;
            }
            else if ((numToFreq.get(num + 1) ?? 0) > 0
                && (numToFreq.get(num + 2) ?? 0) > 0)
            {
                ...
                nextNeededNum.set(num + 3,
                    (nextNeededNum.get(num + 3) ?? 0) + 1);
                break;
            }
            else
            {
