...
const maxFreq = function (s, maxLetters, minSize, maxSize)
{
    ...
    for (let currentSize = minSize; currentSize <= maxSize; currentSize++)
    {
        let left = 0;
        let right = currentSize - 1;

        for (let i = left; i <= right; i++)
        {
            subStringToLetterAmount.set(s[i],
                (subStringToLetterAmount.get(s[i]) ?? 0) + 1);
        }
        if (subStringToLetterAmount.size <= maxLetters)
        {
            const subString = s.slice(left, right + 1);
            subStringToAmount.set(subString, (subStringToAmount.get(subString) ?? 0) + 1);
        }
        ...
        while (right < s.length - 1)
        {
            ...
            subStringToLetterAmount.set(letterRemoved,
            ...
            subStringToLetterAmount.set(letterAdded,
