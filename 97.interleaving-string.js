...
function helper(s1, s1Start, s2, s2Start, s3, s3Start, cache)
{
    ...
    for (let i = s3Start + 1; i <= s3.length; i++)
    {
        ...
        {
            found = found || helper(s1, s1Start + (i - s3Start), s2, s2Start, s3, s3Start + (i - s3Start), cache);
        }
        if (s2.startsWith(prefix, s2Start))
        {
            found = found || helper(s1, s1Start, s2, s2Start + (i - s3Start), s3, s3Start + (i - s3Start), cache);
        }
        if (found)
        {
...
console.log(isInterleave('bcbccabcccbcbbbcbbacaaccccacbaccabaccbabccbabcaabbbccbbbaa', 'ccbccaaccabacaabccaaccbabcbbaacacaccaacbacbbccccbac', 'bccbcccabbccaccaccacbacbacbabbcbccbaaccbbaacbcbaacbacbaccaaccabcaccacaacbacbacccbbabcccccbababcaabcbbcccbbbaa'));    // false
